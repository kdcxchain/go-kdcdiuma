...
{{end}}
RUN \
  echo 'gkdc --cache 512 init /genesis.json' > gkdc.sh && \{{if .Unlock}}
	echo 'mkdir -p /root/.kdcdium/keystore/ && cp /signer.json /root/.kdcdium/keystore/' >> gkdc.sh && \{{end}}
	echo $'exec gkdc --networkid {{.NetworkID}} --cache 512 --port {{.Port}} --nat extip:{{.IP}} --maxpeers {{.Peers}} {{.LightFlag}} --ethstats \'{{.Ethstats}}\' {{if .Bootnodes}}--bootnodes {{.Bootnodes}}{{end}} {{if .Etherbase}}--miner.etherbase {{.Etherbase}} --mine --miner.threads 1{{end}} {{if .Unlock}}--unlock 0 --password /signer.pass --mine{{end}} --miner.gastarget {{.GasTarget}} --miner.gaslimit {{.GasLimit}} --miner.gasprice {{.GasPrice}}' >> gkdc.sh

ENTRYPOINT ["/bin/sh", "gkdc.sh"]
`
...
services:
  {{.Type}}:
    ...
    ports:
      ...
      - "{{.Port}}:{{.Port}}/udp"
    volumes:
      - {{.Datadir}}:/root/.kdcdium{{if .Ethashdir}}
      - {{.Ethashdir}}:/root/.kdchash{{end}}
    environment:
      - PORT={{.Port}}/tcp
...
func checkNode(client *sshClient, network string, boot bool) (*nodeInfos, error) {
	...
	stats := &nodeInfos{
		genesis:    genesis,
		datadir:    infos.volumes["/root/.kdcdium"],
		ethashdir:  infos.volumes["/root/.kdchash"],
		port:       port,
